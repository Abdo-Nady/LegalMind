...
const Card = React.forwardRef(({ className, variant = "default", ...props }, ref) => {
    const variants = {
        ...
        premium: "bg-card text-card-foreground shadow-premium border-secondary/20 hover:border-secondary/40",
        interactive: "bg-card text-card-foreground shadow-premium hover:shadow-card-hover hover:-translate-y-1 cursor-pointer",
    };
    return (<div ref={ref} className={cn("rounded-xl border border-border transition-all duration-300", variants[variant], className)} {...props}/>);
});
...
const CardTitle = React.forwardRef(({ className, ...props }, ref) => (<h3 ref={ref} className={cn("text-xl font-serif leading-none tracking-tight", className)} {...props}/>));
